...
bool TileRenderer::tesselateCrossInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  float sky = getSkyLightRaw(lx, ly, lz, cx, cz, 0, 1, 0);
  float blk = getVertexBlockLight(wX, wY, wZ, 0, 0, 0, 0, 0, 0);
  float brightness = (blk > sky) ? blk : sky;
...
// Block-emission lighting (lava/glowstone/etc), compensated against sun ambient.
