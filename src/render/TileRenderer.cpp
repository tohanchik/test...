...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (id == BLOCK_WATER_STILL || id == BLOCK_WATER_FLOW ||
    ...
    // For flowing liquid columns, suppress the cap on intermediate/falling
    // segments to avoid visible "stacked top planes" in waterfalls.
    if (renderTopFace && (id == BLOCK_WATER_FLOW || id == BLOCK_LAVA_FLOW)) {
      ...
      bool fluidBelow = isFluidId(belowId);
