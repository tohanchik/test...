...
bool TileRenderer::needFace(int lx, int ly, int lz, int cx, int cz, uint8_t id, int dx, int dy, int dz, bool &outIsFancy) {
  ...
  if (g_blockProps[nb].isOpaque()) {
    // Stacked bottom-slabs live in different voxel cells with an air gap
    // between them, so their vertical faces must not be culled.
    if ((dy == 1 || dy == -1) && isSlabBlock(id) && isSlabBlock(nb)) {
      return true;
    }
