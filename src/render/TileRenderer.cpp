...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (isStairId(id)) {
    ...
    // For upside-down stairs, the lower and upper half-bands on vertical faces
    // are mirrored in Y, so pick 1..0.5 for the mirrored "lower" half and
    // 0.5..0 for the mirrored "upper" half.
