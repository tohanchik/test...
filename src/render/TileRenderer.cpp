...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  auto emitOnly = [](float blkL, float skyL) -> float {
    (void)skyL;
    // Visual emissive radius should match physical block-light falloff.
    // Use pure block light here (0..1 mapped from 0..15).
    return blkL > 0.0f ? blkL : 0.0f;
  };
  ...
  if (needFace(lx, ly, lz, cx, cz, id, 0, 1, 0, isFancy)) {
    ...
    if (!g_blockProps[id].isTransparent() && avgEmit > 0.001f) {
