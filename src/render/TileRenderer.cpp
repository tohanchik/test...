...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (isStairBlock(id)) {
    ...
    auto lightForFace = [&](uint32_t base, int dx, int dy, int dz, float mul = 1.0f) -> uint32_t {
      float sl = getSkyLightRaw(lx, ly, lz, cx, cz, dx, dy, dz);
      float bl = getVertexBlockLight(wX + dx, wY + dy, wZ + dz, 0, 0, 0, 0, 0, 0);
      float br = (bl > sl + 0.05f) ? bl : sl;
      // Match the fake AO look used by full cubes: sides are dimmer than top,
      // and bottom is dimmest.
      float dirShade = 1.0f;
      if (dy == 0) dirShade = 0.82f;
      else if (dy < 0) dirShade = 0.65f;

      // Extra contact darkening near solid neighbors (helps stairs blend with
      // adjacent full blocks instead of looking flat-lit).
      int nx = wX + dx, ny = wY + dy, nz = wZ + dz;
      if (ny >= 0 && ny < CHUNK_SIZE_Y) {
        uint8_t nearId = m_level->getBlock(nx, ny, nz);
        if (g_blockProps[nearId].isOpaque()) dirShade *= 0.9f;
      }
      br *= dirShade;
      br *= mul;
      if (br > 1.0f) br = 1.0f;
    ...
    if (needFace(lx, ly, lz, cx, cz, id, 0, 1, 0, isFancy)) {
      uint32_t c = lightForFace(LIGHT_TOP, 0, 1, 0);
      t->addQuad(uTop0, vTop0, uTop1, vTopHalf, c, c, c, c,
    ...
    if (needFace(lx, ly, lz, cx, cz, id, 0, 1, 0, isFancy)) {
      uint32_t c = lightForFace(LIGHT_TOP, 0, 1, 0);
      t->addQuad(uTop0, vTopHalf, uTop1, vTop1, c, c, c, c,
    ...
    // Bottom
    if (needFace(lx, ly, lz, cx, cz, id, 0, -1, 0, isFancy)) {
      uint32_t c = lightForFace(LIGHT_BOT, 0, -1, 0);
      t->addQuad(uBot0, vBot0, uBot1, vBot1, c, c, c, c,
    ...
    if (needFace(lx, ly, lz, cx, cz, id, 0, 0, -1, isFancy)) {
      uint32_t cTop = lightForFace(LIGHT_SIDE, 0, 0, -1, 1.0f);
      uint32_t cBot = lightForFace(LIGHT_SIDE, 0, 0, -1, 0.82f);
      t->addQuad(uSide0, vSideHalf, uSide1, vSide1, cTop, cTop, cBot, cBot,
    ...
    if (needFace(lx, ly, lz, cx, cz, id, 0, 0, 1, isFancy)) {
      uint32_t cTop = lightForFace(LIGHT_SIDE, 0, 0, 1, 1.0f);
      uint32_t cBot = lightForFace(LIGHT_SIDE, 0, 0, 1, 0.82f);
      t->addQuad(uSide0, vSide0, uSide1, vSide1, cTop, cTop, cBot, cBot,
                 wx, wy + 1.0f, wz + 1.0f, wx + 1.0f, wy + 1.0f, wz + 1.0f,
                 wx, wy, wz + 1.0f, wx + 1.0f, wy, wz + 1.0f);
      drawn = true;
    }
    ...
    if (needFace(lx, ly, lz, cx, cz, id, -1, 0, 0, isFancy)) {
      uint32_t cTop = lightForFace(LIGHT_SIDE, -1, 0, 0, 1.0f);
      uint32_t cBot = lightForFace(LIGHT_SIDE, -1, 0, 0, 0.82f);
      t->addQuad(uSide0, vSideHalf, uSide1, vSide1, cTop, cTop, cBot, cBot,
                 wx, wy + 0.5f, wz, wx, wy + 0.5f, wz + 1.0f,
                 wx, wy, wz, wx, wy, wz + 1.0f);
      // West face upper rear half
      t->addQuad(uSideHalf, vSide0, uSide1, vSideHalf, cTop, cTop, cBot, cBot,
    ...
    uint32_t riserTop = lightForFace(LIGHT_SIDE, 0, 0, -1, 1.0f);
    uint32_t riserBot = lightForFace(LIGHT_SIDE, 0, 0, -1, 0.84f);
    t->addQuad(uSide0, vSide0, uSide1, vSideHalf, riserTop, riserTop, riserBot, riserBot,
