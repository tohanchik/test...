...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (id == BLOCK_WATER_STILL || id == BLOCK_WATER_FLOW ||
    ...
    const bool fluidAboveCurrent = (wY + 1 < CHUNK_SIZE_Y) ? isFluidId(m_level->getBlock(wX, wY + 1, wZ)) : false;
    bool drawn = false;
    ...
    auto addSide = [&](int dx, int dz) {
      // For vertical flowing columns (liquid above this cell), skip side walls.
      // This follows the "source-like" look where only the outer/terminal flow
      // layers expose sides, reducing stripy sheets in waterfalls.
      if ((id == BLOCK_WATER_FLOW || id == BLOCK_LAVA_FLOW) && fluidAboveCurrent) return;
