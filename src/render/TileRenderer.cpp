...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (id == BLOCK_WATER_STILL || id == BLOCK_WATER_FLOW ||
    ...
    if (up) {
      ...
      emitVertex(uc - co - s, vc - co + s, c, wx, wy + h00, wz);
      emitVertex(uc - co + s, vc + co + s, c, wx, wy + h01, wz + 1);
      emitVertex(uc + co + s, vc + co - s, c, wx + 1, wy + h11, wz + 1);
      emitVertex(uc + co - s, vc - co - s, c, wx + 1, wy + h10, wz);
      drawn = true;
    }
    ...
    if (down) {
      ...
      emitVertex(u0, v0, c, wx, wy, wz + 1);
      emitVertex(u1, v0, c, wx + 1, wy, wz + 1);
      emitVertex(u1, v1, c, wx + 1, wy, wz);
      emitVertex(u0, v1, c, wx, wy, wz);
      drawn = true;
    }
    ...
    for (int face = 0; face < 4; face++) {
      ...
      emitVertex(u0, v01, c, x0, wy + hh0, z0);
      emitVertex(u1, v02, c, x1, wy + hh1, z1);
      emitVertex(u1, v1, c, x1, wy, z1);
      emitVertex(u0, v1, c, x0, wy, z0);
      drawn = true;
    }
    return drawn;
  }
