...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (isStairId(id)) {
    ...
    auto stairLitColor = [&](uint32_t base, int sx, int sy, int sz,
      ...
      float br = sky;
      if (blk > 0.001f) {
        float boost = blk * 1.35f;
        if (boost > 1.0f) boost = 1.0f;
        if (boost > br) br = boost;
      }
      if (stairBlkCenter > 0.001f) {
        float cBoost = stairBlkCenter * 1.35f;
        if (cBoost > 1.0f) cBoost = 1.0f;
        if (cBoost > br) br = cBoost;
      }
    ...
    auto stairEmitColor = [&](uint32_t base, int sx, int sy, int sz,
                              ...
                              int dx2, int dy2, int dz2) -> uint32_t {
      float blk = getVertexBlockLight(sx, sy, sz, dx1, dy1, dz1, dx2, dy2, dz2);
      if (stairBlkCenter > blk) blk = stairBlkCenter;
      float e = blk * 1.35f;
      if (e > 1.0f) e = 1.0f;
