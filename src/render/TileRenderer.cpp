...
bool TileRenderer::tesselateCrossInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  uint32_t emitCol = applyLightToFace(baseColor, blkL);
...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (id == BLOCK_WATER_STILL || id == BLOCK_WATER_FLOW ||
    ...
    // Self-lit fluids (lava) are drawn in emissive pass so the block itself
    // stays bright at night instead of being dimmed by transparent ambient.
    Tesselator *fluidTess = selfLitFluid ? m_emitTess : m_transTess;
  ...
  auto emitOnly = [&](float blkL, float skyL) -> float {
    ...
    if (selfLitBlock || transparentBlock) return blkL;
