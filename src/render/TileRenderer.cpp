...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (isStairId(id)) {
    ...
    // Stair lighting rebuilt from scratch: compute each vertex color from the
    // actual face normal + vertex position so front/rear neighboring solids
    // only affect the vertices they geometrically touch.
    auto stairVertexColor = [&](uint32_t base, float vx, float vy, float vz,
                                int nx, int ny, int nz, bool emissive) -> uint32_t {
      int sx = (vx < 0.5f) ? -1 : ((vx > 0.5f) ? 1 : 0);
      int sy = (vy < 0.5f) ? -1 : ((vy > 0.5f) ? 1 : 0);
      int sz = (vz < 0.5f) ? -1 : ((vz > 0.5f) ? 1 : 0);
      ...
      float sky = getVertexSkyLight(ax, ay, az, dx1, dy1, dz1, dx2, dy2, dz2);
      float blk = getVertexBlockLight(ax, ay, az, dx1, dy1, dz1, dx2, dy2, dz2);
    ...
    auto stairFaceColor = [&](uint32_t base, float vx, float vy, float vz,
                              int nx, int ny, int nz) -> uint32_t {
      return stairVertexColor(base, vx, vy, vz, nx, ny, nz, false);
    };
    auto stairFaceEmitColor = [&](uint32_t base, float vx, float vy, float vz,
                                  int nx, int ny, int nz) -> uint32_t {
      return stairVertexColor(base, vx, vy, vz, nx, ny, nz, true);
    };

    uint32_t topC00 = stairFaceColor(LIGHT_TOP, 0.0f, 1.0f, 0.0f, 0, 1, 0);
    uint32_t topC10 = stairFaceColor(LIGHT_TOP, 1.0f, 1.0f, 0.0f, 0, 1, 0);
    uint32_t topC01 = stairFaceColor(LIGHT_TOP, 0.0f, 1.0f, 1.0f, 0, 1, 0);
    uint32_t topC11 = stairFaceColor(LIGHT_TOP, 1.0f, 1.0f, 1.0f, 0, 1, 0);
    uint32_t botC00 = stairFaceColor(LIGHT_BOT, 0.0f, 0.0f, 0.0f, 0,-1, 0);
    uint32_t botC10 = stairFaceColor(LIGHT_BOT, 1.0f, 0.0f, 0.0f, 0,-1, 0);
    uint32_t botC01 = stairFaceColor(LIGHT_BOT, 0.0f, 0.0f, 1.0f, 0,-1, 0);
    uint32_t botC11 = stairFaceColor(LIGHT_BOT, 1.0f, 0.0f, 1.0f, 0,-1, 0);
    uint32_t northC11 = stairFaceColor(LIGHT_SIDE, 1.0f, 1.0f, 0.0f, 0, 0,-1);
    uint32_t northC01 = stairFaceColor(LIGHT_SIDE, 0.0f, 1.0f, 0.0f, 0, 0,-1);
    uint32_t northC10 = stairFaceColor(LIGHT_SIDE, 1.0f, 0.0f, 0.0f, 0, 0,-1);
    uint32_t northC00 = stairFaceColor(LIGHT_SIDE, 0.0f, 0.0f, 0.0f, 0, 0,-1);
    uint32_t southC01 = stairFaceColor(LIGHT_SIDE, 0.0f, 1.0f, 1.0f, 0, 0, 1);
    uint32_t southC11 = stairFaceColor(LIGHT_SIDE, 1.0f, 1.0f, 1.0f, 0, 0, 1);
    uint32_t southC00 = stairFaceColor(LIGHT_SIDE, 0.0f, 0.0f, 1.0f, 0, 0, 1);
    uint32_t southC10 = stairFaceColor(LIGHT_SIDE, 1.0f, 0.0f, 1.0f, 0, 0, 1);
    uint32_t westC01 = stairFaceColor(LIGHT_SIDE, 0.0f, 1.0f, 0.0f,-1, 0, 0);
    uint32_t westC11 = stairFaceColor(LIGHT_SIDE, 0.0f, 1.0f, 1.0f,-1, 0, 0);
    uint32_t westC00 = stairFaceColor(LIGHT_SIDE, 0.0f, 0.0f, 0.0f,-1, 0, 0);
    uint32_t westC10 = stairFaceColor(LIGHT_SIDE, 0.0f, 0.0f, 1.0f,-1, 0, 0);
    uint32_t eastC11 = stairFaceColor(LIGHT_SIDE, 1.0f, 1.0f, 1.0f, 1, 0, 0);
    uint32_t eastC01 = stairFaceColor(LIGHT_SIDE, 1.0f, 1.0f, 0.0f, 1, 0, 0);
    uint32_t eastC10 = stairFaceColor(LIGHT_SIDE, 1.0f, 0.0f, 1.0f, 1, 0, 0);
    uint32_t eastC00 = stairFaceColor(LIGHT_SIDE, 1.0f, 0.0f, 0.0f, 1, 0, 0);

    uint32_t eTopC00 = stairFaceEmitColor(LIGHT_TOP, 0.0f, 1.0f, 0.0f, 0, 1, 0);
    uint32_t eTopC10 = stairFaceEmitColor(LIGHT_TOP, 1.0f, 1.0f, 0.0f, 0, 1, 0);
    uint32_t eTopC01 = stairFaceEmitColor(LIGHT_TOP, 0.0f, 1.0f, 1.0f, 0, 1, 0);
    uint32_t eTopC11 = stairFaceEmitColor(LIGHT_TOP, 1.0f, 1.0f, 1.0f, 0, 1, 0);
    uint32_t eBotC00 = stairFaceEmitColor(LIGHT_BOT, 0.0f, 0.0f, 0.0f, 0,-1, 0);
    uint32_t eBotC10 = stairFaceEmitColor(LIGHT_BOT, 1.0f, 0.0f, 0.0f, 0,-1, 0);
    uint32_t eBotC01 = stairFaceEmitColor(LIGHT_BOT, 0.0f, 0.0f, 1.0f, 0,-1, 0);
    uint32_t eBotC11 = stairFaceEmitColor(LIGHT_BOT, 1.0f, 0.0f, 1.0f, 0,-1, 0);
    uint32_t eNorthC11 = stairFaceEmitColor(LIGHT_SIDE, 1.0f, 1.0f, 0.0f, 0, 0,-1);
    uint32_t eNorthC01 = stairFaceEmitColor(LIGHT_SIDE, 0.0f, 1.0f, 0.0f, 0, 0,-1);
    uint32_t eNorthC10 = stairFaceEmitColor(LIGHT_SIDE, 1.0f, 0.0f, 0.0f, 0, 0,-1);
    uint32_t eNorthC00 = stairFaceEmitColor(LIGHT_SIDE, 0.0f, 0.0f, 0.0f, 0, 0,-1);
    uint32_t eSouthC01 = stairFaceEmitColor(LIGHT_SIDE, 0.0f, 1.0f, 1.0f, 0, 0, 1);
    uint32_t eSouthC11 = stairFaceEmitColor(LIGHT_SIDE, 1.0f, 1.0f, 1.0f, 0, 0, 1);
    uint32_t eSouthC00 = stairFaceEmitColor(LIGHT_SIDE, 0.0f, 0.0f, 1.0f, 0, 0, 1);
    uint32_t eSouthC10 = stairFaceEmitColor(LIGHT_SIDE, 1.0f, 0.0f, 1.0f, 0, 0, 1);
    uint32_t eWestC01 = stairFaceEmitColor(LIGHT_SIDE, 0.0f, 1.0f, 0.0f,-1, 0, 0);
    uint32_t eWestC11 = stairFaceEmitColor(LIGHT_SIDE, 0.0f, 1.0f, 1.0f,-1, 0, 0);
    uint32_t eWestC00 = stairFaceEmitColor(LIGHT_SIDE, 0.0f, 0.0f, 0.0f,-1, 0, 0);
    uint32_t eWestC10 = stairFaceEmitColor(LIGHT_SIDE, 0.0f, 0.0f, 1.0f,-1, 0, 0);
    uint32_t eEastC11 = stairFaceEmitColor(LIGHT_SIDE, 1.0f, 1.0f, 1.0f, 1, 0, 0);
    uint32_t eEastC01 = stairFaceEmitColor(LIGHT_SIDE, 1.0f, 1.0f, 0.0f, 1, 0, 0);
    uint32_t eEastC10 = stairFaceEmitColor(LIGHT_SIDE, 1.0f, 0.0f, 1.0f, 1, 0, 0);
    uint32_t eEastC00 = stairFaceEmitColor(LIGHT_SIDE, 1.0f, 0.0f, 0.0f, 1, 0, 0);
