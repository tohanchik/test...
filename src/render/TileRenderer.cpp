...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (id == BLOCK_WATER_STILL || id == BLOCK_WATER_FLOW ||
    ...
    bool hasHorizontalWaterNeighbor = false;
    if (isWater) {
      hasHorizontalWaterNeighbor =
          isFluidId(m_level->getBlock(wX - 1, wY, wZ)) ||
          isFluidId(m_level->getBlock(wX + 1, wY, wZ)) ||
          isFluidId(m_level->getBlock(wX, wY, wZ - 1)) ||
          isFluidId(m_level->getBlock(wX, wY, wZ + 1));
    }
    if (isWater && selfWaterDepth == 0 && !hasWaterAbove && !hasHorizontalWaterNeighbor) {
