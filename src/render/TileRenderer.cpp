...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (isStairBlock(id)) {
    ...
    auto lightForFace = [&](uint32_t base, int dx, int dy, int dz) -> uint32_t {
    ...
    if (needFace(lx, ly, lz, cx, cz, id, 0, 0, -1, isFancy)) {
      uint32_t c = lightForFace(LIGHT_SIDE, 0, 0, -1);
      t->addQuad(uSide0, vSideHalf, uSide1, vSide1, c, c, c, c,
    ...
    if (needFace(lx, ly, lz, cx, cz, id, 0, 0, 1, isFancy)) {
      uint32_t c = lightForFace(LIGHT_SIDE, 0, 0, 1);
      t->addQuad(uSide0, vSide0, uSide1, vSide1, c, c, c, c,
                 wx, wy + 1.0f, wz + 1.0f, wx + 1.0f, wy + 1.0f, wz + 1.0f,
                 wx, wy, wz + 1.0f, wx + 1.0f, wy, wz + 1.0f);
      drawn = true;
    }
    ...
    if (needFace(lx, ly, lz, cx, cz, id, -1, 0, 0, isFancy)) {
      uint32_t c = lightForFace(LIGHT_SIDE, -1, 0, 0);
      t->addQuad(uSide0, vSideHalf, uSide1, vSide1, c, c, c, c,
                 wx, wy + 0.5f, wz, wx, wy + 0.5f, wz + 1.0f,
                 wx, wy, wz, wx, wy, wz + 1.0f);
      // West face upper rear half
      t->addQuad(uSideHalf, vSide0, uSide1, vSideHalf, c, c, c, c,
    ...
    uint32_t riser = lightForFace(LIGHT_SIDE, 0, 0, -1);
