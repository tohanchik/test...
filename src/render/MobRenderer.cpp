...
static inline UVRect uvRect(const BoxDef &box, int face, float invTexW, float invTexH) {
  ...
  UVRect uv;
  uv.u0 = (x0 + eps) * invTexW;
  uv.v0 = (y0 + eps) * invTexH;
  uv.u1 = (x1 - eps) * invTexW;
  uv.v1 = (y1 - eps) * invTexH;
  return uv;
}
...
static inline void addBox(MobVertex *verts, int &v, const BoxDef &b,
  ...
  addFace(verts, v, x1, y0, z1, x1, y0, z0, x1, y1, z0, x1, y1, z1, uvRect(b, 0, invTexW, invTexH), col); // right
  addFace(verts, v, x0, y0, z0, x0, y0, z1, x0, y1, z1, x0, y1, z0, uvRect(b, 1, invTexW, invTexH), col); // left
  addFace(verts, v, x0, y1, z0, x1, y1, z0, x1, y1, z1, x0, y1, z1, uvRect(b, 2, invTexW, invTexH), col); // top
  addFace(verts, v, x0, y0, z1, x1, y0, z1, x1, y0, z0, x0, y0, z0, uvRect(b, 3, invTexW, invTexH), col); // bottom
  addFace(verts, v, x0, y0, z0, x1, y0, z0, x1, y1, z0, x0, y1, z0, uvRect(b, 4, invTexW, invTexH), col); // front
  addFace(verts, v, x1, y0, z1, x0, y0, z1, x0, y1, z1, x1, y1, z1, uvRect(b, 5, invTexW, invTexH), col); // back
}
...
void MobRenderer::render(const MobSystem &mobs, const Player *player, float sunBrightness) {
  ...
  const float invTexW = (m_pigTexture.width > 0) ? (1.0f / (float)m_pigTexture.width) : 1.0f;
  const float invTexH = (m_pigTexture.height > 0) ? (1.0f / (float)m_pigTexture.height) : 1.0f;
  ...
  for (size_t i = 0; i < visible.size(); ++i) {
    ...
    addBox(verts, v, kHead, m.x, m.y, m.z, col, invTexW, invTexH);
    addBox(verts, v, kSnout, m.x, m.y, m.z, col, invTexW, invTexH);
    addBox(verts, v, kBody, m.x, m.y, m.z, col, invTexW, invTexH);
    addBox(verts, v, kLegFL, m.x, m.y, m.z, col, invTexW, invTexH);
    addBox(verts, v, kLegFR, m.x, m.y, m.z, col, invTexW, invTexH);
    addBox(verts, v, kLegBL, m.x, m.y, m.z, col, invTexW, invTexH);
    addBox(verts, v, kLegBR, m.x, m.y, m.z, col, invTexW, invTexH);
  }
