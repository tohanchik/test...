...
static void renderFallingBlocks() {
  ...
  for (size_t i = 0; i < falling.size(); ++i) {
    ...
    FallingBlockVertex *verts = (FallingBlockVertex *)sceGuGetMemory(24 * sizeof(FallingBlockVertex));
    int v = 0;
    auto addQuad = [&](float ax, float ay, float az, float bx, float by, float bz,
    ...
    // Top
    addQuad(x0, y1, z0, x1, y1, z0, x1, y1, z1, x0, y1, z1, tu0, tv0, tu1, tv1, 0xFFFFFFFF);
    // Bottom
    addQuad(x0, y0, z1, x1, y0, z1, x1, y0, z0, x0, y0, z0, bu0, bv0, bu1, bv1, 0xFFBFBFBF);
    // North (-Z)
    addQuad(x0, y0, z0, x1, y0, z0, x1, y1, z0, x0, y1, z0, su0, sv1, su1, sv0, 0xFFDDDDDD);
    // South (+Z)
    addQuad(x1, y0, z1, x0, y0, z1, x0, y1, z1, x1, y1, z1, su0, sv1, su1, sv0, 0xFFDDDDDD);
    // West (-X)
    addQuad(x0, y0, z1, x0, y0, z0, x0, y1, z0, x0, y1, z1, su0, sv1, su1, sv0, 0xFFCCCCCC);
    // East (+X)
    addQuad(x1, y0, z0, x1, y0, z1, x1, y1, z1, x1, y1, z0, su0, sv1, su1, sv0, 0xFFCCCCCC);

    sceGuDrawArray(GU_QUADS,
