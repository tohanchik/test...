...
static void game_render() {
  ...
  // First-person camera anti-clipping:
  // 1) slight forward bias (MCPE-like feel, reduces edge peeking at zero distance),
  // 2) if camera ends up inside a solid AABB, pull it back along view ray.
  if (g_level && g_player) {
    const float camForwardBias = 0.08f;
    camPos.x += lookDir.x * camForwardBias;
    camPos.y += lookDir.y * camForwardBias;
    camPos.z += lookDir.z * camForwardBias;

    auto pointInsideBlockAABB = [&](float px, float py, float pz) -> bool {
      int bx = (int)floorf(px);
      int by = (int)floorf(py);
      int bz = (int)floorf(pz);
      uint8_t bid = g_level->getBlock(bx, by, bz);
      if (bid == BLOCK_AIR) return false;
      const BlockProps &bp = g_blockProps[bid];
      if (!bp.isSolid()) return false;
    ...
    if (pointInsideBlockAABB(camPos.x, camPos.y, camPos.z)) {
      const float pullStep = 0.08f;
      for (int i = 0; i < 10; ++i) {
        camPos.x -= lookDir.x * pullStep;
        camPos.y -= lookDir.y * pullStep;
        camPos.z -= lookDir.z * pullStep;
        if (!pointInsideBlockAABB(camPos.x, camPos.y, camPos.z)) break;
      }
    }
  }
