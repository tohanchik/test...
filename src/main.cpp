...
static void drawHUD() {
  ...
  if (g_player && g_player->isInventoryOpen()) {
    ...
    const float titleW = g_font.getStringWidth(inv.categoryName(), 1.0f);
    const float titleX = titleAreaX + (titleAreaW - titleW) * 0.5f + kInvTitleOffsetX;
    g_font.drawShadow(titleX, titleY, inv.categoryName(), 0xFF303030, 1.0f);
    ...
    if (hoverName) {
      ...
      if (hoverX + g_font.getStringWidth(hoverName, 1.0f) > 475.0f) hoverX = cursorX - g_font.getStringWidth(hoverName, 1.0f) - 8.0f;
      g_font.drawShadow(hoverX, hoverY, hoverName, 0xFFFFFFFF, 1.0f);
    }
