...
static void drawHUD() {
  ...
  if (g_player && g_player->isInventoryOpen()) {
    ...
    if (g_guiInvCreativeTex.data) hudDrawTexture(g_guiInvCreativeTex, panelX, panelY, panelW, panelH);
    else hudDrawRect(45.0f, 16.0f, 390.0f, 240.0f, 0xD0202020);

    const float cellSize = 19.0f;
    const float gridX0 = 125.0f;
    const float gridY0 = 36.0f;
    const float stepX = 21.3f;
    const float stepY = 20.3f;
    const float iconPad = 1.5f;
    const float iconSize = cellSize - iconPad * 2.0f;
    const int itemsPerPage = 50;

    int base = inv.creativePage() * itemsPerPage;
    for (int r = 0; r < 5; ++r) {
      for (int c = 0; c < 10; ++c) {
        float sx = gridX0 + c * stepX;
        float sy = gridY0 + r * stepY;
        ...
        int idx = base + r * 10 + c;
        if (idx >= inv.visibleItemCount()) continue;
    ...
    const float hotbarX0 = 125.0f;
    for (int i = 0; i < 9; ++i) {
      float sx = hotbarX0 + i * stepX;
      ...
      if (i == inv.hotbarSel()) {
        hudDrawRect(sx - 1.0f, hotbarY - 1.0f, cellSize + 2.0f, cellSize + 2.0f, 0xA0FFFF40);
      }
    }

    float cursorX = gridX0 + inv.cursorX() * stepX;
    float cursorY = (inv.cursorY() < 5) ? (gridY0 + inv.cursorY() * stepY) : hotbarY;
    ...
    if (g_guiSliderTex.data) {
      int maxPage = inv.visibleItemCount() / 50;
      float t = (maxPage > 0) ? ((float)inv.creativePage() / (float)maxPage) : 0.0f;
      float sliderY = 41.0f + t * 78.0f;
      hudDrawTexture(g_guiSliderTex, 377.0f, sliderY, 15.0f, 15.0f);
    }
