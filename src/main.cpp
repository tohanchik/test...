...
static void beginIsometricScrShotMode() {
  ...
  setStatusMessage("Isometric /scr mode: 100 ticks", 2.5f, 0xFF80FF80);
}
      ...
      if (g_isoShotActive && g_chunkRenderer) {
        ...
        if (g_isoShotTick < kIsoShotTicks) {
          ...
          if (g_isoShotTick == kIsoShotTicks) {
            ...
            g_chunkRenderer->setRenderDistanceOverride(-1.0f);
            setStatusMessage("Isometric /scr finished, camera restored", 3.0f, 0xFF80FF80);
          }
        }
      }
