...
static void renderFallingBlocks() {
  ...
  for (size_t i = 0; i < falling.size(); ++i) {
    ...
    int ex = (int)floorf(e.x);
    int ey = (int)floorf(e.y);
    int ez = (int)floorf(e.z);
    if (isWaterId(g_level->getBlock(ex, ey, ez))) visibleInWaterCount++;
    visibleCount++;
  }
  ...
  FallingBlockVertex *waterVerts = visibleInWaterCount > 0
      ? (FallingBlockVertex *)sceGuGetMemory((size_t)visibleInWaterCount * 36 * sizeof(FallingBlockVertex))
      : nullptr;
  int v = 0, vw = 0;
  auto addFace = [&](float ax, float ay, float az, float bx, float by, float bz,
                     float cx, float cy, float cz, float dx, float dy, float dz,
                     float u0, float v0, float u1, float v1, uint32_t col, FallingBlockVertex *dst, int &idx) {
    // Triangle 1: A-B-C
    dst[idx++] = {u0, v0, col, ax, ay, az};
    dst[idx++] = {u1, v0, col, bx, by, bz};
    dst[idx++] = {u1, v1, col, cx, cy, cz};
    // Triangle 2: A-C-D
    dst[idx++] = {u0, v0, col, ax, ay, az};
    dst[idx++] = {u1, v1, col, cx, cy, cz};
    dst[idx++] = {u0, v1, col, dx, dy, dz};
  };
  ...
  for (size_t i = 0; i < falling.size(); ++i) {
    ...
    int ey = (int)floorf(e.y);
    int ez = (int)floorf(e.z);
    bool inWater = isWaterId(g_level->getBlock(ex, ey, ez));
    FallingBlockVertex *dst = inWater ? waterVerts : verts;
    int &idx = inWater ? vw : v;

    // Top
    addFace(x0, y1, z0, x1, y1, z0, x1, y1, z1, x0, y1, z1, tu0, tv0, tu1, tv1, 0xFFFFFFFF, dst, idx);
    // Bottom
    addFace(x0, y0, z1, x1, y0, z1, x1, y0, z0, x0, y0, z0, bu0, bv0, bu1, bv1, 0xFFBFBFBF, dst, idx);
    // North (-Z)
    addFace(x0, y0, z0, x1, y0, z0, x1, y1, z0, x0, y1, z0, su0, sv1, su1, sv0, 0xFFDDDDDD, dst, idx);
    // South (+Z)
    addFace(x1, y0, z1, x0, y0, z1, x0, y1, z1, x1, y1, z1, su0, sv1, su1, sv0, 0xFFDDDDDD, dst, idx);
    // West (-X)
    addFace(x0, y0, z1, x0, y0, z0, x0, y1, z0, x0, y1, z1, su0, sv1, su1, sv0, 0xFFCCCCCC, dst, idx);
    // East (+X)
    addFace(x1, y0, z0, x1, y0, z1, x1, y1, z1, x1, y1, z0, su0, sv1, su1, sv0, 0xFFCCCCCC, dst, idx);
  ...
  if (vw > 0) {
    ...
    if (g_player) {
      int cx = (int)floorf(g_player->getX());
      int cy = (int)floorf(g_player->getY() + 1.62f);
      int cz = (int)floorf(g_player->getZ());
      cameraInWater = isWaterId(g_level->getBlock(cx, cy, cz));
    }
    if (cameraInWater) sceGuDisable(GU_DEPTH_TEST);
    sceGuDrawArray(GU_TRIANGLES,
                   GU_TEXTURE_32BITF | GU_COLOR_8888 | GU_VERTEX_32BITF | GU_TRANSFORM_3D,
                   vw, 0, waterVerts);
    if (cameraInWater) sceGuEnable(GU_DEPTH_TEST);
  }
