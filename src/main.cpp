...
static void game_render() {
  ...
  if (g_player && !g_player->isFlyingCreative()) {
    ...
    const float xOff = Mth::sin(b * Mth::PI) * bobAmt * 0.5f;
    const float yOff = -fabsf(Mth::cos(b * Mth::PI) * bobAmt);
    const float rollDeg = Mth::sin(b * Mth::PI) * bobAmt * 3.0f;
    const float pitchDeg = fabsf(Mth::cos(b * Mth::PI - 0.2f) * bobAmt) * 5.0f + tiltAmt;
