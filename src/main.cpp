...
static inline void hudDrawStairIso(TextureAtlas *atlas, uint8_t id, float x, float y, float size) {
  ...
  // Upper back half-block (top stair step), shifted up and slightly to the back/left.
  const float ux0 = cx - size * 0.14f;
  const float uy0 = y;
  const float ux1 = x + size * 0.62f;
  const float uy1 = y + topHalfH * 1.00f;
  const float ux2 = cx - size * 0.14f;
  const float uy2 = y + topHalfH * 2.00f;
  const float ux3 = x - size * 0.02f;
  const float uy3 = y + topHalfH * 1.00f;
