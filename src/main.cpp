...
static bool captureFrameBufferBMP(char *outPath, size_t outPathSize) {
  ...
  int bufferWidth = 0;
  int pixelFormat = 0;
  if (sceDisplayGetFrameBuf(&framePtr, &bufferWidth, &pixelFormat, PSP_DISPLAY_SETBUF_IMMEDIATE) < 0) return false;
  if (!framePtr || bufferWidth <= 0 || pixelFormat != PSP_DISPLAY_PIXEL_FORMAT_8888) return false;

  sceIoMkdir("screenshots", 0777);
  unsigned long long ts = (unsigned long long)sceKernelGetSystemTimeWide();
  snprintf(outPath, outPathSize, "screenshots/isometric_%llu.bmp", ts);
  ...
  const uint32_t *src = (const uint32_t *)framePtr;
  for (int y = height - 1; y >= 0; --y) {
    int dstI = 0;
    const uint32_t *srcRow = src + y * bufferWidth;
...
static void game_update(float dt) {
  ...
  // Rare debug combo for classic-style isometric screenshot.
  // Can be moved to UI later.
