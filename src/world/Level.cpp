...
void Level::setBlock(int wx, int wy, int wz, uint8_t id) {
  ...
  if (m_deferLightingUpdates) return;

  // Full relight only when a light source changes (placed/removed/changed).
  // Otherwise keep fast incremental update.
  if (g_blockProps[oldId].light_emit != g_blockProps[id].light_emit) {
    computeLighting();
  } else {
