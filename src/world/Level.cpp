...
void Level::computeLighting() {
  ...
  // 1) CrossCraft World::update_lighting(x, z) logic port:
  //    - clear column skylight
  //    - retrace from top
  //    - mark transparent blocks lit and continue
  //    - mark first blocking block lit and stop
  for (int x = 0; x < WORLD_CHUNKS_X * CHUNK_SIZE_X; x++) {
    for (int z = 0; z < WORLD_CHUNKS_Z * CHUNK_SIZE_Z; z++) {
      for (int y = 0; y < CHUNK_SIZE_Y; y++) setSkyLight(x, y, z, 0);

      for (int y = CHUNK_SIZE_Y - 1; y >= 0; y--) {
        uint8_t id = getBlock(x, y, z);
        if (id == BLOCK_AIR || id == BLOCK_FLOWER || id == BLOCK_ROSE ||
