...
void Level::tickFallingBlocks() {
  ...
  for (size_t i = 0; i < m_fallingBlocks.size(); ++i) {
    ...
    // In liquids, the block should keep falling "as in air" until it reaches
    // solid support. Avoid force-placing after a short timeout because that can
    // create temporary moving air pockets and repeated re-spawn behavior.
    if (e.age > 600) {
      e.removed = true;
    }
  }
